...
void load_train_file( 
	CascadeClassifier &face_det,				/* in : face detector */
	...
	vector<vector<shape_type> > &shapes)	/* out: shapes */
{
	...
	for(; iterImageElement; iterImageElement = iterImageElement->NextSiblingElement()) //image section
	{
		...
		for (; iterBox; iterBox = iterBox->NextSiblingElement())
		{
			...
			/*show */
			vector<Rect> results;
			face_det.detectMultiScale(input_img, results, 1.1, 2, 0, Size(30,30));


			for (unsigned long i=0;i<results.size();i++)
			{
				//cv::rectangle( input_img, results[i], Scalar(255,255,255), 2);
...
int main( int argc, char** argv)
{
	...
	load_train_file( face_detector, train_file_xml, image_root, 256, imgs, rects, shapes);
	...
	sp.save_model("haar_shape_model.xml");
	sp.load_model("haar_shape_model.xml");
	...
	load_train_file( face_detector, test_file_path, image_root,256, imgs, rects, shapes);
